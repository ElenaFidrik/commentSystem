var CommentActive = /** @class */ (function () {
    function CommentActive() {
        ...
        this.resultComment = document.getElementById('result_comment');
        this.answerBody = document.getElementById('idAnswer');
    }
    // обработчик кнопки
    CommentActive.prototype.sending = function () {
        var _a;
        (_a = this.btn) === null || _a === void 0 ? void 0 : _a.addEventListener('click', function () {
            comm.commentContent();
        });
    };
    CommentActive.prototype.commentContent = function () {
        this.comment = {
            answer: [],
            body: this.textarea.value,
            time: Math.floor(Date.now() / 1000),
            userSend: 'Максим Авдеенко',
            photoSend: 'img/maks.png',
            like: false,
            favoriteOff: 'В избранное',
            ratingScore: 0
        };
        input.count.innerText = "\u041C\u0430\u043A\u0441. ".concat(input.limit, " \u0441\u0438\u043C\u0432\u043E\u043B\u043E\u0432");
        input.btn.style.backgroundColor = '#dbd7d7';
        this.textarea.value = '';
        if (this.comment.body.length != '' && this.comment.body.length <= input.limit) {
    ...
    CommentActive.prototype.saveComments = function () {
        localStorage.setItem('comments', JSON.stringify(this.comments));
    };
    ;
    // отображаем из localStorage
    CommentActive.prototype.localComments = function () {
    ...
    CommentActive.prototype.deleteComments = function () {
        localStorage.clear();
    };
    ;
    // отрисовка  коммента
    CommentActive.prototype.drawComments = function () {
    ...
    ;
    //время и дата коммента
    CommentActive.prototype.timeConverter = function (UNIX_timestamp) {
    ...
    ;
    //отрисовка сердечка
    CommentActive.prototype.drawHeart = function (like) {
        var htmlHeart = '';
        if (like) {
    ...
    ;
    //обработчик лайка 
    CommentActive.prototype.toggleHeart = function () {
        document.querySelectorAll('.inFavorite').forEach(function (item) {
            item.addEventListener("click", function (event) {
                ...
                if (favoriteBtn.classList.contains("toggleHeart")) {
                    favoriteBtn.innerHTML = comm.drawHeart(true);
                    comm.comments[index].like = true;
                }
                else if (!favoriteBtn.classList.contains("toggleHeart")) {
                    favoriteBtn.innerHTML = comm.drawHeart(false);
    ...
    ;
    //обработчик рейтинга
    CommentActive.prototype.changeRating = function () {
